...
def data_abstraction(begin,end, ignore):
    """"
    ...
    set_output="D:\\User\\Documents\\School\\Thesis\\thesis_sieben_bocklandt\\Data\\pdf_data_usa"
    ...
    for data_index in range(begin,end):
        if data_index not in ignore:
            ...
            responsive_data = np.loadtxt(output_directory + "\\responsive_scores.dat")
            try:
                if len(data[0])==2:
                    scores = data[:, 0]
