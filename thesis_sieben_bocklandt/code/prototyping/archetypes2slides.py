...
def archetypes2slides(archetypes, tree, output_directory,ppt_path, RA=None, make_slides=True):
    ...
    for page in tree.getroot():
        ...
        if isinstance(archetype,TitleSlide):
            ...
            slide_triple_content(title, left_content,middle_content, right_content, page_size,slide_height,slide_width,
            ...
            slide_background_quote(title,background,slide,output_directory, slide_height,slide_width,page_size,used_content)
        elif isinstance(archetype, BackgroundOnly):
            placeholder_percentage.append(1 / nb_elements)
            background=good_elements[archetype.background]
            to_remove_as_good_element.add(archetype.background)
            slide=prs.slides.add_slide(prs.slide_layouts[BACKGROUND_ONLY])
            slide_background_only(background,slide,output_directory, slide_height,slide_width,page_size,used_content)
            ...
            slide_content_only(contents,prs.slide_width,prs.slide_height,page_size,slide,output_directory,used_content)
        else:
            print(archetype)
            raise TypeError
        # if RA!=None:
        ...
        for i in range(0,len(good_elements)):
            if i not in to_remove_as_good_element:
                ...
                add_content_without_placeholder(content,bbox,slide,output_directory,slide_height,x_factor,y_factor,used_content)
    ...
    for slide in prs.slides:
        ...
        txBox = slide.shapes.add_textbox(footer_dimensions[1],footer_dimensions[0],footer_dimensions[2],footer_dimensions[3])
        tf = txBox.text_frame
        tf.text = ref
        tf.paragraphs[0].font.size = footer_font
        if add_numbers:
            if (counter==0 and slide_numbers[0]) or counter> 0:
                txBox_slidenumber = slide.shapes.add_textbox(slide_number_dimensions[1], slide_number_dimensions[0], slide_number_dimensions[2],
...
def slide_single_content(titletext,content,page_size,slide_height,slide_width,slide, output_directory,used_content):
    """
...
def slide_double_content(titletext,left_content,right_content,page_size,slide_height,slide_width,slide, output_directory,used_content):
    """
    ...
    title.text = titletext
    used_content.append(titletext)
    add_content(slide, left_content_placeholder, left_content, output_directory, slide_height,slide_width,page_size,used_content)
    add_content(slide, right_content_placeholder, right_content, output_directory, slide_height,slide_width,page_size,used_content)

def slide_triple_content(titletext,left_content,middle_content,right_content,page_size,slide_height,slide_width,slide,output_directory,used_content):
    """
    ...
    add_content(slide,left_content_placeholder,left_content,output_directory, slide_height,slide_width,page_size,used_content)
    add_content(slide, middle_content_placeholder, middle_content, output_directory, slide_height,slide_width,page_size,used_content)
    add_content(slide, right_content_placeholder, right_content, output_directory, slide_height,slide_width,page_size,used_content)

def slide_comparison(titletext,left_subtitle,right_subtitle,left_content,right_content,page_size,slide_height,slide_width, slide,output_directory,used_content):
    """
    ...
    add_content(slide,left_subtitle_placeholder,left_subtitle,output_directory, slide_height,slide_width,page_size,used_content)
    add_content(slide,right_subtitle_placeholder,right_subtitle,output_directory, slide_height,slide_width,page_size,used_content)
    add_content(slide,left_content_placeholder,left_content,output_directory, slide_height,slide_width,page_size,used_content)
    add_content(slide,right_content_placeholder,right_content,output_directory, slide_height,slide_width,page_size,used_content)
...
def slide_captioned_content(top_content,left_content,right_content,slide,output_directory, slide_height,slide_width,page_size,used_content):
    """
    ...
    add_content(slide,top_content_placeholder,top_content,output_directory, slide_height,slide_width,page_size,used_content)
    add_content(slide, left_content_placeholder, left_content,output_directory, slide_height,slide_width,page_size,used_content)
    add_content(slide, right_content_placeholder, right_content,output_directory, slide_height,slide_width,page_size,used_content)

def slide_background_quote(titletext,background,slide,output_directory, slide_height,slide_width,page_size,used_content):
    """
    ...
    add_content(slide,background_placeholder,background,output_directory, slide_height,slide_width,page_size,used_content)
...
def slide_content_only(contents,slide_width,slide_height,page_size,slide,output_directory,used_content):
    ...
    for content in contents:
        add_content_without_placeholder(content,[float(x) for x in content.attrib.get("bbox").split(",")], slide,output_directory,slide_height,x_factor,y_factor,used_content)


def add_content_without_placeholder(content,bbox,slide,output_directory,slide_height, x_factor,y_factor,used_content):
    """
    ...
    if content.tag == "picture" or content.tag=="background":
        ...
        table = slide.shapes.add_table(table_height, table_width, int(bbox[0] * x_factor), int(slide_height - y_factor * bbox[3]),
